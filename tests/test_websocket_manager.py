...
class MockWebSocket:
    ...
    def __init__(self, state="CONNECTED"):
        ...
        self.messages = []
        self.closed = False
        self.application_state = Mock()
        self.application_state.value = "CONNECTED" if state == "CONNECTED" else "DISCONNECTED"
...
class TestWebSocketManager:
    ...
    async def test_connect_client_success(self, websocket_manager, mock_websocket):
        ...
        with patch('src.api.websocket_manager.jwt.decode') as mock_decode:
            ...
            # Mock WebSocket receive_text to return auth token
            mock_websocket.receive_text = AsyncMock(return_value='{"type": "auth", "data": {"token": "test_token"}}')
    ...
    async def test_unsubscribe_from_task(self, websocket_manager):
        ...
        assert "test-client" not in websocket_manager.task_subscriptions["test-task"]
    ...
    async def test_unsubscribe_from_bid(self, websocket_manager):
        ...
        assert "test-client" not in websocket_manager.bid_subscriptions["test-bid"]
    ...
    async def test_send_system_alert(self, websocket_manager):
        ...
        assert message1["data"]["alert_type"] == "maintenance"
        assert message2["data"]["alert_type"] == "maintenance"
...
class TestWebSocketAPIEndpoints:
    ...
    def test_subscribe_task_endpoint(self, client: TestClient, test_task):
        ...
        response = client.post("/api/ws/subscribe/task", json={
            "task_id": test_task.id
        })
    ...
    def test_subscribe_bid_endpoint(self, client: TestClient, test_bid):
        ...
        response = client.post("/api/ws/subscribe/bid", json={
            "bid_id": test_bid.id
        })
        # Should return 401 Unauthorized without authentication
        assert response.status_code == 401
