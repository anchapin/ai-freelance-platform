...
class FinancialSummaryTemplate:
    ...
    def _build_financial_document(self):
        ...
        # Subtitle/Date
        subtitle = content.get("subtitle", f"Report Date: {datetime.now().strftime('%B %d, %Y')}")
    ...
    def _add_key_metrics(self, metrics: Dict[str, Any]):
        ...
        if metrics:
            ...
            for i, (metric_name, metric_value) in enumerate(metrics.items(), 1):
                ...
                row[1].text = self._format_currency(metric_value) if self._is_currency(metric_name) else str(metric_value)
    ...
    def _add_highlights(self, highlights: List[Dict[str, Any]]):
        ...
        for highlight in highlights:
            if isinstance(highlight, dict):
                ...
                # Change (optional)
                if change:
                    "green" if "+" in str(change) or "increase" in str(change).lower() else "red"
        ...
        for row_idx, row_data in enumerate(rows):
            ...
            for col_idx, value in enumerate(row_data):
                if col_idx < len(row.cells):
                    ...
                    cell.text = self._format_currency(value) if self._should_format_currency(headers[col_idx]) else str(value)
    ...
    def _is_currency(self, metric_name: str) -> bool:
        ...
        currency_keywords = ["revenue", "income", "profit", "loss", "cost", "expense", "balance", "amount", "value", "total"]
