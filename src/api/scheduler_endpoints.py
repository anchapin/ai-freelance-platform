...
async def schedule_task(
    ...
    try:
        ...
        return {
            ...
            "human_readable": CronExpressionValidator.get_human_readable(request.cron_expression)
        }
...
async def schedule_daily_task_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to schedule daily task: {e}")
...
async def schedule_weekly_task_endpoint(
    ...
    try:
        ...
        cron_expr = f"{time_of_day.split(':')[1]} {time_of_day.split(':')[0]} * * {day_of_week}"
        ...
        raise HTTPException(status_code=500, detail=f"Failed to schedule weekly task: {e}")
...
async def schedule_monthly_task_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to schedule monthly task: {e}")
...
async def get_schedule(
    ...
    try:
        ...
        return {
            ...
            "human_readable": CronExpressionValidator.get_human_readable(schedule.cron_expression)
        }
...
async def get_schedule_analytics(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get schedule analytics: {e}")
...
async def get_schedule_history(
    ...
    try:
        ...
        history_query = db.query(ScheduleHistory).filter(
            ScheduleHistory.schedule_id == schedule_id
        ).order_by(ScheduleHistory.execution_start.desc()).offset(offset).limit(limit)
...
async def get_scheduler_status(
    ...
    try:
        ...
        recent_executions = db.query(func.count(ScheduleHistory.id)).filter(
