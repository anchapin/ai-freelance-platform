...
class BackupRequest(BaseModel):
    ...
    backup_type: str = Field(..., description="Type of backup: full, incremental, point_in_time")
    force: bool = Field(False, description="Force backup even if not scheduled")
    timestamp: Optional[str] = Field(None, description="Timestamp for point-in-time backup")
...
class RecoveryRequest(BaseModel):
    ...
    target_location: Optional[str] = Field(None, description="Target location for recovery")
...
class DisasterRecoveryRequest(BaseModel):
    ...
    disaster_type: str = Field(..., description="Type of disaster: database_corruption, data_loss, system_failure, ransomware_attack")
    plan_id: str = Field("default", description="Recovery plan to use")
...
class DisasterRecoveryAPI:
    ...
    async def create_backup(
        ...
        try:
            ...
            if backup_type_enum == BackupType.FULL:
                ...
                if timestamp:
                    try:
                        backup_timestamp = datetime.fromisoformat(timestamp.replace('Z', '+00:00'))
                    except ValueError:
                        raise HTTPException(status_code=400, detail="Invalid timestamp format. Use ISO format.")
                else:
                    backup_timestamp = None
                metadata = await self.backup_manager.create_point_in_time_backup(timestamp=backup_timestamp)
            else:
                raise HTTPException(status_code=400, detail=f"Unsupported backup type: {backup_type}")
    ...
    async def list_backups(
        ...
        try:
            ...
            # Get backups
            backups = await self.backup_manager.list_backups(backup_type=backup_type_enum)
    ...
    async def delete_backup(self, backup_id: str) -> Dict[str, str]:
        ...
        try:
            ...
            metadata_file = self.backup_manager.backup_dir / f"{backup_id}_metadata.json"
    ...
    async def execute_recovery(
        ...
        try:
            ...
            return RecoveryResponse(
                ...
                end_time=recovery_op.end_time.isoformat() if recovery_op.end_time else None,
    ...
    async def get_recovery_status(self, operation_id: str) -> Optional[RecoveryResponse]:
        """
    ...
    async def execute_disaster_recovery(
        ...
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Disaster recovery failed: {str(e)}")
    ...
    async def get_recovery_plans(self) -> List[RecoveryPlanResponse]:
        ...
        try:
            ...
            for plan_id, plan in self.recovery_manager.recovery_plans.items():
                plans.append(RecoveryPlanResponse(
            ...
            raise HTTPException(status_code=500, detail=f"Failed to get recovery plans: {str(e)}")
...
async def get_backup_details_endpoint(
    backup_id: str,
    db: Session = Depends(get_db)
):
    """
...
async def test_recovery_plan_endpoint(
    plan_id: str,
    db: Session = Depends(get_db)
):
    """
...
async def get_recovery_plans_endpoint(
    db: Session = Depends(get_db)
):
    """
    Get available recovery plans.
...
async def health_check_endpoint(
    ...
    try:
        ...
        status = "healthy" if all([backup_dir_ok, db_ok, redis_ok, s3_ok]) else "degraded"
