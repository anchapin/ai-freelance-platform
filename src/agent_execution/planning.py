...
class ContextExtractor:
    ...
    def extract_context(
        ...
        if (
            self.llm
            and context.get("extraction_success")
            and (file_content or csv_data)
        ):
    ...
    def execute_workflow(
        ...
        if approved:
            ...
            workflow_result["message"] = (
                f"Artifact not approved after {review_attempts} attempts: {current_feedback}"
            )
