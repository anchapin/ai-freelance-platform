...
class IntelligentScheduler:
    ...
    def get_optimal_time(self, base_time: datetime, schedule: ScheduledTask) -> datetime:
        """Calculate optimal execution time avoiding peak hours."""
        if not self.should_avoid_peak_hours(schedule):
            return base_time
        
        # If base time is in peak hours, move to next available slot
        hour = base_time.hour
        is_peak = any(start <= hour < end for start, end in self.peak_hours)
        
        if is_peak:
            # Move to end of peak hours (5 PM)
            next_time = base_time.replace(hour=17, minute=0, second=0, microsecond=0)
            if next_time <= base_time:
                # If we're past 5 PM, move to tomorrow morning
                next_time = base_time.replace(hour=6, minute=0, second=0, microsecond=0) + timedelta(days=1)
            return next_time
...
class TaskScheduler:
    ...
    async def schedule_task(
        ...
        next_run = self.intelligent_scheduler.get_optimal_time(next_run, type('obj', (object,), {
        ...
        try:
            ...
            logger.info(f"Task scheduled: {schedule.id} - {title} - Next run: {next_run}")
            return schedule.id
        except Exception as e:
    ...
    async def resume_schedule(self, schedule_id: str) -> bool:
        ...
        try:
            ...
            if schedule:
                ...
                next_run = self.intelligent_scheduler.get_optimal_time(next_run, schedule)
    ...
    async def get_schedule_analytics(self, schedule_id: str) -> Dict[str, Any]:
        ...
        history_query = select(ScheduleHistory).where(
        ...
        return {
            ...
            "human_readable": self.validator.get_human_readable(schedule.cron_expression),
            "recent_executions": [
                {
                    "execution_start": h.execution_start.isoformat(),
                    "execution_end": h.execution_end.isoformat() if h.execution_end else None,
                    "status": h.status,
    ...
    async def _execute_schedule(self, schedule: ScheduledTask):
        ...
        try:
            ...
            if callback:
                # Parse task data
                task_data = json.loads(schedule.task_data) if hasattr(schedule, 'task_data') else {}
            ...
            if schedule.avg_execution_time == 0:
                ...
                schedule.avg_execution_time = (
                    (schedule.avg_execution_time + execution_time) / 2
                )
            ...
            successful_runs = len([h for h in [history_record] if h.status == "COMPLETED"])
            schedule.success_rate = (successful_runs / total_runs * 100) if total_runs > 0 else 100.0
            ...
            history_record.execution_end = datetime.now()
    ...
    async def _update_next_run_times(self):
        ...
        for schedule in schedules:
            try:
                ...
                logger.error(f"Failed to update next run time for schedule {schedule.id}: {e}")
