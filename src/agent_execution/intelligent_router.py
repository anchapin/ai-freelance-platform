...
class TaskClassifier:
    ...
    def train(self, task_profiles: List[TaskProfile], labels: List[str]):
        ...
        self.clustering_model = KMeans(n_clusters=min(5, len(set(labels))), random_state=42)
...
class PerformanceTracker:
    ...
    def record_execution(self, task_profile: TaskProfile, actual_success: bool):
        ...
        if task_profile.execution_time:
            self.handler_performance[handler]['execution_times'].append(task_profile.execution_time)
        self.handler_performance[handler]['complexities'].append(task_profile.complexity_score)
    ...
    def _update_metrics(self, handler: str):
        ...
        if data['successes']:
            self.metrics['success_rate'][handler] = sum(data['successes']) / len(data['successes'])
        
        if data['execution_times']:
            self.metrics['avg_execution_time'][handler] = np.mean(data['execution_times'])
    ...
    def get_handler_recommendations(self, task_profile: TaskProfile) -> List[Dict[str, Any]]:
        """
...
class IntelligentRouter:
    ...
    async def route_task(
        ...
        **kwargs
        ...
        performance_recommendations = self.performance_tracker.get_handler_recommendations(task_profile)
        ...
        # Record performance
        self.performance_tracker.record_execution(task_profile, result.get('success', False))
    ...
    def _create_task_profile(
        ...
        return TaskProfile(
            ...
            output_format=output_format or self.task_router.detect_output_format(domain, task_type or "visualization"),
    ...
    def _calculate_success_rate(self, domain: str, task_type: str, output_format: str) -> float:
        """
    ...
    def _make_routing_decision(
        ...
        reasoning = self._generate_reasoning(
            task_profile, classification, performance_recommendations, best_handler, is_anomaly
        )
        
        # Determine fallback handlers
        fallback_handlers = [
            rec['handler'] for rec in performance_recommendations[:3]
    ...
    def _generate_reasoning(
        ...
        if classification.get('method') == 'ml_classification':
            ...
            reasoning_parts.append("Used rule-based classification due to untrained model")
        
        # Performance reasoning
        perf_rec = next(
            (r for r in performance_recommendations if r['handler'] == selected_handler),
            None
        )
        if perf_rec:
            reasoning_parts.append(
        ...
        try:
            ...
            for fallback_handler in decision.fallback_handlers:
                try:
                    ...
                    logger.error(f"Fallback execution failed for handler {fallback_handler}: {fallback_error}")
                    continue
    ...
    async def _execute_legal_task(self, task_profile: TaskProfile, **kwargs) -> Dict[str, Any]:
    ...
    async def _execute_accounting_task(self, task_profile: TaskProfile, **kwargs) -> Dict[str, Any]:
    ...
    async def _execute_visualization_task(self, task_profile: TaskProfile, **kwargs) -> Dict[str, Any]:
    ...
    async def _execute_document_task(self, task_profile: TaskProfile, **kwargs) -> Dict[str, Any]:
    ...
    async def _execute_spreadsheet_task(self, task_profile: TaskProfile, **kwargs) -> Dict[str, Any]:
    ...
    async def _execute_report_task(self, task_profile: TaskProfile, **kwargs) -> Dict[str, Any]:
    ...
    async def _execute_standard_task(self, task_profile: TaskProfile, **kwargs) -> Dict[str, Any]:
    ...
    def get_routing_analytics(self) -> Dict[str, Any]:
        ...
        performance_recommendations = self.performance_tracker.get_handler_recommendations(sample_profile)
        
        return {
            'total_tasks_routed': sum(self.performance_tracker.metrics['task_volume'].values()),
            'average_success_rate': np.mean(list(self.performance_tracker.metrics['success_rate'].values())) if self.performance_tracker.metrics['success_rate'] else 0.0,
            'top_performing_handlers': performance_recommendations[:3],
            'handler_distribution': dict(self.performance_tracker.metrics['task_volume']),
            'classification_accuracy': np.mean(self.classifier.classification_metrics['training_accuracy']) if self.classifier.classification_metrics['training_accuracy'] else 0.0,
...
def get_intelligent_router(db_session=None, model_path: Optional[str] = None) -> IntelligentRouter:
    """
