    ...
    def calculate_profit_score(
        ...
        if is_local:
            ...
            if "gpt-4o" in model.lower() and "mini" not in model.lower():
                ...
                ) * self.cost_config.CLOUD_GPT4O_INPUT_COST
                output_cost = (
                    total_output_tokens / 1_000_000
                ) * self.cost_config.CLOUD_GPT4O_OUTPUT_COST
            else:
                # GPT-4o-mini: separate input and output rates
                input_cost = (
                    total_input_tokens / 1_000_000
                ) * self.cost_config.CLOUD_GPT4O_MINI_INPUT_COST
                output_cost = (
                    total_output_tokens / 1_000_000
                ) * self.cost_config.CLOUD_GPT4O_MINI_OUTPUT_COST
    ...
    async def execute(
        ...
        try:
            ...
            return {
                ...
                "approved": result.get("steps", {})
                .get("artifact_review", {})
                .get("approved", False),
                "feedback": result.get("steps", {})
                .get("artifact_review", {})
                .get("feedback", ""),
...
class ArenaRouter:
    ...
    async def run_arena(
        ...
        arena_result = {
            ...
            "winning_artifact_url": result_a.get("artifact_url")
            if winner == "agent_a"
            else result_b.get("artifact_url"),
...
class ArenaLearningLogger:
    ...
    def log_loser(self, arena_result: Dict[str, Any], task_data: Dict[str, Any]):
        ...
        dpo_example = {
            ...
            # Metadata
            "profit_diff": winner_data["profit"]["profit"]
            - loser_data["profit"]["profit"],
...
if __name__ == "__main__":
    ...
    async def example():
        ...
        logger.info(
            f"Profit Breakdown - Revenue: ${profit['revenue'] / 100:.2f}, "
